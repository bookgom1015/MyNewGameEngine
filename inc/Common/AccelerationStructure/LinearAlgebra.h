...
namespace Common::AccelerationStructure {
	struct Vector3Df {
		...
		__host__ __device__ Vector3Df(float _x = 0.f, float _y = 0.f, float _z = 0.f) 
			: x(_x), y(_y), z(_z) {}
		__host__ __device__ Vector3Df(const Vector3Df& v) 
			: x(v.x), y(v.y), z(v.z) {}
		__host__ __device__ Vector3Df(const float4& v) 
			: x(v.x), y(v.y), z(v.z) {}
