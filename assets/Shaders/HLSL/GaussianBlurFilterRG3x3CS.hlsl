#ifndef __GAUSSIANBLURFILTERRG3X3CS_HLSL__
#define __GAUSSIANBLURFILTERRG3X3CS_HLSL__
...
Texture2D<float2>   gi_InputMap  : register(t0);
RWTexture2D<float2> go_OutputMap : register(u0);
...
void CS(in uint2 dispatchThreadID : SV_DispatchThreadID) {
    ...
    float2 samples[4];
    samples[0] = gi_InputMap.SampleLevel(gsamLinearMirror, (dispatchThreadID + offsets[0]) * gInvTexDim, 0);
    samples[1] = gi_InputMap.SampleLevel(gsamLinearMirror, (dispatchThreadID + offsets[1]) * gInvTexDim, 0);
    samples[2] = gi_InputMap.SampleLevel(gsamLinearMirror, (dispatchThreadID + offsets[2]) * gInvTexDim, 0);
    samples[3] = gi_InputMap[dispatchThreadID + 1];
    ...
    go_OutputMap[dispatchThreadID] = float2(dot(samplesR, weights), dot(samplesG, weights));
}
#else 
...
#endif // __GAUSSIANBLURFILTERRG3X3CS_HLSL__