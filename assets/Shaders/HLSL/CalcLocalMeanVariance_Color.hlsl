...
void FilterHorizontally(uint2 Gid, uint GI) {
    ...
    for (uint i = 0; i < NumRowsToLoadPerThread; ++i) {
        ...
        float4 value = ShadingConvention::SVGF::InvalidColorValueW;
		...
		// Filter the values for the first GroupDim columns.
		{
            ...
            for (uint c = 0; c < cbLocalMeanVar.KernelRadius; ++c) {
                ...
                const float4 cValue = WaveReadLaneAt(value, LaneToReadFrom);
...
void FilterVertically(uint2 DTid, uint2 GTid) {
    ...
    for (uint r = 0; r < cbLocalMeanVar.KernelWidth; ++r) {
        ...
        if (rNumValues > 0) {
            ...
            const float4 rValueSum = ValuePackaging::UintToFloat4(UnpackedRowSum.x);
            const float4 rSquaredValueSum = ValuePackaging::UintToFloat4(UnpackedRowSum.y);
    ...
    const float4 Mean = InvN * valueSum;
    ...
    go_LocalMeanVariance[Pixel] = numValues > 0 ? float2(Packed, variance) : ShadingConvention::SVGF::InvalidColorValueW;
}
