...
#ifndef __COUNTINGSORT_RAYS_64X128_HLSL__
#define __COUNTINGSORT_RAYS_64X128_HLSL__
...
uint CreateIndexHashKey(in uint2 rayIndex) {
    if (INDEX_HASH_KEY_BITS == 0) return 0;
...
void SpillCachedIndicesToVRAMAndCacheInvertedSortedIndices(in uint2 Gid, in uint GI) {
    ...
    for (uint index = GI; index < NumRays; index += NUM_THREADS) {
        ...
        go_SortedToSourceRayIndexOffsetMap[outPixel] = UnflattenRayIndex(packedSrcIndex); // Output the source index for this sorted ray index.
    }
}
...
#endif // __COUNTINGSORT_RAYS_64X128_HLSL__