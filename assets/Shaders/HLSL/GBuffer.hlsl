...
void MS(
    ...
    for (uint i = 0; i < 3; ++i) {
        ...
        const float4 PosW = mul(float4(vout.PosL, 1.f), cbObject.World);
        vout.PosW = PosW.xyz;
        ...
        const float4 PrevPosW = mul(float4(vin.Position, 1.f), cbObject.PrevWorld);
        vout.PrevPosH = mul(PrevPosW, cbPass.PrevViewProj);
        
        vout.NormalW = mul(vin.Normal, (float3x3)cbObject.World);
        vout.PrevNormalW = mul(vin.Normal, (float3x3)cbObject.PrevWorld);
        
        const float4 TexC = mul(float4(vin.TexCoord, 0.f, 1.f), cbObject.TexTransform);
        vout.TexC = TexC.xy;
